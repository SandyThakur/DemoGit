...
struct EmpDetailEnvelop:Requestable {
    var apiPath:String {return "/emp_detail.php"}
    var httpType:HttpType {return .post}
    var pathType : ServicePath
    var modelType:Identifiable.Type? {return EmpUserInfo.self}
}
...
internal enum ServicePath:ParameterBodyMaker {
    ...
    case employeeDetail(emp_ID:String)
    ...
    func httpBodyEnvelop()->[String:Any]? {
        ...
        case .employeeDetail(emp_ID: let eID):
            let postBody = ["ID":eID]
            return postBody
        }
